...
group_1
    Comma-separated replicate-level concentration values for the group_1.

group_2
    Comma-separated replicate-level concentration values for the group_2.

EXAMPLE INPUT
-------------
common_id	common_name	group_1	group_2
...
The output is a tab-delimited file containing the following columns:
    ...
    p_value
    fdr
    log2fc # log2(group_2/group_1)
    regulation # if group_2 > group_1 regulation is "up", else regulation is "down"
    significant
...
def safe_log2fc(mean_g1, mean_g2):
    """
    Calculate log2(g2 / GC).
    ...
    pseudo = 1e-9
    g1 = g1 if g1 > 0 else pseudo
    g2 = g2 if g2 > 0 else pseudo
...
def run_dam_analysis(rows, fdr_threshold):
    ...
    for row in rows:
        ...
        list_g1 = parse_numeric_list(row['group_1'])
        list_g2 = parse_numeric_list(row['group_2'])

        if (not list_g1) or (not list_g2):
            continue

        mean_g1 = safe_mean(list_g1)
        mean_g2 = safe_mean(list_g2)

        if mean_g1 is None or mean_g2 is None:
            continue

        u_stat, p_value = mannwhitneyu(list_g1, list_g2, correction=False)
    ...
    for metabolite_id in order:
        ...
        log2fc = safe_log2fc(mean_g1, mean_g2)
...
def main():
    ...
    if results:
        ...
        top = results[0]
        for key in [
            'common_id', 'common_name', 'n_g1', 'n_g2', 'mean_g1', 'mean_g2',
